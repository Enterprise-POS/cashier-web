...
function StoreProvider({ children }: { children: React.ReactNode }) {
	...
	function editStore(storeDef: StoreDef) {
		// The same id is not allowed so StoreDef is here to confirm no same id at Store list
		setStoreState(val => ({
			...val,
			storeList: val.storeList.map(store => (store.id === storeDef.id ? new Store(storeDef) : store)),
		}));
	}
